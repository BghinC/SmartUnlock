...
.run(function($ionicPlatform, $rootScope, constellationConsumer) {
  ...
  constellationConsumer.initializeClient("http://192.168.43.180:8888", "5f9ee889c65b65fb74dec90bff902ea285582d17", "Application Ionic");
    constellationConsumer.onConnectionStateChanged(function (change) {
        if (change.newState === $.signalR.connectionState.connected) {
            console.log("Connecté à constellation");
            $rootScope.isConnected = true;
            constellationConsumer.registerStateObjectLink("*", "SerrurePackage", "Serrure", "*", function(so) {
              $rootScope.Serrure = so.Value;
              $rootScope.$apply();
            });
        }
    });
